...
const TrainerNavbar = () => {
  ...
  const trainerName = useAppSelector((state) => state.auth.name || 'Trainer');
    ...
    <nav className="bg-gray-900 w-full fixed top-0 left-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-8">
            <NavLink to="/trainer" onClick={() => setMenuOpen(false)} className="text-2xl font-extrabold text-red-500 cursor-pointer" style={{ textDecoration: 'none' }}>
              Blaze Gym
            </NavLink>
            <div className="hidden sm:flex space-x-4">
              <NavLink to="/trainer" end className={navLinkClasses}>Home</NavLink>
              <NavLink to="/trainer/registeredMembers" className={navLinkClasses}>Registered Members</NavLink>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden sm:flex flex-col items-center">
              <span className="text-gray-300 font-medium text-sm">Hi, {trainerName}</span>
              <button
      ...
      {menuOpen && (
        <div className="sm:hidden bg-gray-800 shadow-lg px-4 pb-4 pt-2 space-y-3 rounded-b-md">
          {[
            { to: '/trainer', label: 'Home' },
            { to: '/trainer/registeredMembers', label: 'Registered Members' },
          ].map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
          ...
          <div className="border-t border-gray-700 pt-3 text-center">
            <p className="text-gray-300 text-sm">Hi, {trainerName}</p>
            <button
              onClick={() => {
                setMenuOpen(false);
                handleLogout();
